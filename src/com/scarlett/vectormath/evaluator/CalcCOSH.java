 ...
 *
 */
public class CalcCOSH extends Calc1ParamFunctionEvaluator {
	
	@Override
	protected CalcObject evaluateObject(CalcObject input) {
		if (input instanceof CalcVector) {
			throw new CalcWrongParametersException("COSH is not defined for vectors.");
		}
	...
	private CalcObject evaluateDegree(CalcNumber input) {
		return new CalcDouble(Math.cosh(input.doubleValue()/180*Math.PI));
	}
	
	@Override
	protected CalcObject evaluateDouble(CalcDouble input) {
		return new CalcDouble(Math.cosh(input.doubleValue()));
	}
	...
	protected CalcObject evaluateFunction(CalcFunction input) {
		return CALC.COSH.createFunction(input);
	}

	@Override
	protected CalcObject evaluateInteger(CalcInteger input) {
		return new CalcDouble(Math.cosh(input.bigIntegerValue().intValue()));
	}
